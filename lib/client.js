...
module.exports = class CommandoClient extends Client
{
    ...
    constructor(options)
    {
        ...
        if (options.schedulers.enabled) this.schedulers = new SchedulerManager(this, { path: options.schedulers.path });
        if (options.localization.enabled) this.localization = new LocalizationManager(this, { path: options.localization.path });
        if (options.inhibitors.enabled) this.inhibitors = new InhibitorManager(this, { path: options.inhibitors.path });
    ...
    getPrefixes(content)
    {
        ...
        const prefixes = this.prefixes[i].replace(/@mention/g, `${uwu}`);
        return prefixes;
    }

    /**
     * Uses a preset
     * @param {import('./entities/preset')} preset The preset instance
     */
    use(preset)
    {
 ...
 * @prop {import('./entities/dialect')} [dialect] The dialect instance
 * 
